...
class HomePage:
    ...
    def searching_result_should_be(self, transaction_name):
        browser.all('[id="transactions-tbody"] .transaction .payee strong')[0].should(
            have.exact_text(transaction_name)
        )
        return self
    ...
    def fill_transaction_name(self, transaction_name):
        browser.element('input[name="receiver"][id="expense-receiver"]').should(
            be.visible
        ).click().set_value(transaction_name)
        return self

    @allure.step("Fill transaction amount: '{transaction_amount}'")
    def fill_transaction_amount(self, transaction_amount):
        browser.element('input[name="amount"][id="expense-amount"]').should(
            be.visible
        ).click().set_value(transaction_amount)
        return self

    @allure.step("Select envelope by UUID")
    def set_transaction_envelope(self, envelope_uuid):
        browser.element(
            f'.controls.envelope [name="envelopeUuid"] [value="{envelope_uuid}"]'
        ).click()
        return self
    ...
    def choose_envelope(self, envelope_name):
        browser.all('[id="wrapper-envelopes"] .nodes .name').element_by(
            have.exact_text(envelope_name)
        ).click()
        return self
