...
from utils.api_helpers import (
    get_transactions_by_envelope_uuid, get_existing_transaction_data, get_all_envelopes
)
...
class ApiSteps:
    ...
    def assert_transaction_present(self, session_cookie, credentials, envelope_uuid, txn_name ):
    ...
    def assert_transaction_not_present(self,session_cookie, credentials, envelope_uuid, txn_uuid):
    ...
    def get_txn_data(self, transaction_uuid,credentials, session_cookie):
        response = get_existing_transaction_data(transaction_uuid, credentials, session_cookie)
        return response

    @allure.step('GET created transactions list')
    def get_envelope_data(self, session_cookie, credentials, envelope_uuid):
        response = get_transactions_by_envelope_uuid(session_cookie, credentials, envelope_uuid)
        return response
